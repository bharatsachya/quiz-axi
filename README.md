...
After an agent finishes a change, it writes a short quiz about that diff, opens it in your browser, and waits. You read the diff, answer the questions, and the agent grades you live. **Highlight any passage and ask about it** — your question reaches the agent with that passage attached, and the answer lands in a thread you can follow up in. The page follows your system light/dark theme, with a toggle that sticks. Get everything right and the review closes itself. A **husky `pre-push` hook** then refuses `git push` for any diff that hasn't been reviewed and passed — so the check isn't just a suggestion, it's enforced at the door.
...
| `quiz-axi poll <diff_key> [--agent-reply "..." [--thread <id>]]` | Long-poll until the human answers, asks something, or ends the session. `--thread` attaches the reply to the question thread it answers. |
...
- **Anchored questions**: highlighting prose or diff lines mints a thread carrying that passage. The browser's offsets are never trusted — the server re-resolves every anchor against its own parse of the diff and the quiz spec, and *rebuilds* a diff anchor's quoted text from its own hunk lines. An anchor that no longer resolves (a re-review whose corrected `quiz.json` rewrote the prose under it) is flagged stale and widened to the whole block rather than dropped, so the question still reaches the agent with the quote the human actually saw.
- **Threads without a migration**: `session.chat` stays a flat array; entries simply gained an optional `thread_id`, with a `threads` sidecar holding each thread's anchor. A chat array written before threads existed has none of those fields and renders exactly as it always did.
- **Browser code is real ES modules, still with no bundler.** Pure logic lives in `src/client/*.js`, served over HTTP *and* imported by `node:test`, so the tested code is the shipped code. A module graph aborts on a failed import, so `STATIC_ASSETS` is an explicit allowlist and a test asserts every import resolves to a served route.
- Session state lives in `~/.quiz-axi/state.json` as two maps: `sessions` (live review state — chat, threads, answers, score) and `review_index` (the sealed pass/fail record `verify` reads).
